...
<!--#include file="carrellocalcolo.inc" -->
<%
...
' Totale di Spesa					
Sql="select id,sessionid,codice_isp,descrizione,sum(quantita) as quantita,sum(prezzo) as prezzo,data,prezzou,blocco,fornitore,iva,aliqiva from Carrello where Sessionid='" &  Session.SessionID & "'  GROUP BY codice_isp Order by codice_isp desc,ID Desc"
...
Do While Not tbcar.EOF
	...
	if Trim(tbcar("iva"))="si" then											
		...
		if tbcar("fornitore")="ecommerce" then 
	...
	if lcase(trim(tbcar("iva")))="no" then 
	   HTML = HTML   &  "<td width='50'  align='right'   ><b>" &  dicituraiva & "</td></tr>"
	else
	...
	'Storico 
	StrSqlst1="Insert into Storico (Utente1,Password1,Codice_Isp,Descrizione,Quantita,Prezzounita,Prezzototale,Iva,Data,Tempo,Fornitore,Numeroordine,Tipoordine,Aliqiva) values ('" & Trim(text_utente) & "','" & Trim(text_password) & "','" & tbcar("codice_isp") & "','" & descrizione & "'," & tbcar("quantita") & "," & Replace(tbcar("prezzou"),",",".") & "," & Replace(tbcar("prezzo"),",",".") & ",'" & tbcar("iva") & "',current_date,'" & t_time & "','" & tbcar("fornitore") & "','" & numeroordine & "','O'," & tbcar("aliqiva") & ")"		   
	connm.Execute(StrSqlst1)
	'Ordini SQL
